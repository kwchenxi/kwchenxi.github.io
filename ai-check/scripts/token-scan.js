#!/usr/bin/env node
/**
 * Token Scan — Design Token 静态扫描工具
 * 扫描 Electron 应用（app.asar 解包后）或 Web 项目的 CSS/JS 文件，
 * 检测 Design Token 使用率与硬编码色值/间距/圆角/字体/阴影。
 *
 * 用法:
 *   node token-scan.js <目录>                              # 扫描已解包的目录（使用内置默认值）
 *   node token-scan.js <目录> --config spec.json           # 从 spec 文件读取 Token 定义
 *   node token-scan.js <目录> --config walkthrough-config.json  # 从走查配置读取（自动定位 spec）
 *   node token-scan.js <目录> --html                       # 同时生成 HTML 报告
 *   node token-scan.js <目录> --json out.json              # 输出 JSON 到文件
 *   node token-scan.js <目录> --token-prefix --my-         # 自定义 Token 变量前缀
 *
 * 解包 app.asar:
...
const DEFAULT_CONFIG = {
  ...
  // Token 变量前缀（用于生成正则）
  tokenPrefix: '--lg-',
  // Token 前缀别名（classifyColorContext 中识别的 token 定义前缀）
  tokenAliases: ['--lg-', '--yui-', '--lingee-', '--rv-'],
  // 扫描文件扩展名
...
// 运行时配置（初始为默认值，--config 加载后合并）
let CONFIG = { ...DEFAULT_CONFIG };

// ─── 配置加载 ─────────────────────────────────────────────────

/**
 * 从 spec JSON（lingee-walkthrough-spec.json 格式）提取 Token 值
 */
function loadFromSpec(specPath) {
  if (!fs.existsSync(specPath)) {
    console.error(`⚠ 配置文件不存在：${specPath}，使用默认值`);
    return {};
  }
  ...
  // 判断是 walkthrough-config.json 还是 spec.json
  // walkthrough-config.json 有 design.specFile 字段指向真正的 spec
  if (spec.design && spec.design.specFile) {
    const realSpecPath = path.resolve(path.dirname(specPath), spec.design.specFile);
    if (fs.existsSync(realSpecPath)) {
      const realSpec = JSON.parse(fs.readFileSync(realSpecPath, 'utf-8'));
      return loadFromSpecContent(realSpec, spec);
    } else {
      console.error(`⚠ specFile 指向的文件不存在：${realSpecPath}，使用默认值`);
      return {};
    }
  }

  // 直接是 spec.json
  return loadFromSpecContent(spec, null);
}

/**
 * 从 spec 内容提取 token 配置
 */
function loadFromSpecContent(spec, walkConfig) {
  const extracted = {};
  const tokens = spec.design_tokens;

  if (!tokens) return extracted;
  ...
  if (tokens.color) {
    ...
    function collectColors(obj) {
      if (typeof obj === 'string' && /^#[0-9a-fA-F]{3,8}$/.test(obj)) {
        colors.push(obj.slice(1).toUpperCase());
      } else if (typeof obj === 'object' && obj !== null) {
        Object.values(obj).forEach(collectColors);
      }
    }
    collectColors(tokens.color);
    if (colors.length > 0) extracted.knownTokenColors = colors;
  }

  // 提取 spacing 值
  if (tokens.spacing && tokens.spacing.tokens) {
    const vals = Object.values(tokens.spacing.tokens).filter(v => typeof v === 'number');
    if (vals.length > 0) extracted.spacingTokenValues = [...new Set(vals)].sort((a, b) => a - b);
  }

  // 提取 radius 值
  if (tokens.radius && tokens.radius.tokens) {
    const vals = Object.values(tokens.radius.tokens)
      .map(t => typeof t === 'object' ? t.px : t)
      .filter(v => typeof v === 'number');
    if (vals.length > 0) extracted.radiusTokenValues = [...new Set(vals)].sort((a, b) => a - b);
  }

  // 提取 typography font-sizes
  if (tokens.typography) {
    const sizes = new Set();
    const typo = tokens.typography;
    if (typo.heading) Object.values(typo.heading).forEach(h => { if (h.size) sizes.add(h.size); });
    if (typo.paragraph) Object.values(typo.paragraph).forEach(p => { if (p.size) sizes.add(p.size); });
    if (sizes.size > 0) extracted.fontSizeTokenValues = [...sizes].sort((a, b) => a - b);
  }

  // 从 components 推断 token 前缀（取第一个 figma_node 所在组件的 var 前缀）
  // 如果 spec 有 source 或 config 指定了前缀
  if (walkConfig && walkConfig.design && walkConfig.design.tokenPrefix) {
    extracted.tokenPrefix = walkConfig.design.tokenPrefix;
  }
...
/**
 * 根据 CONFIG.tokenPrefix 动态构建正则
 */
function buildRegexes(prefix) {
  // 转义前缀中的特殊字符用于正则
  const esc = prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

  return {
    // Token 使用（好）
    tokenVar: new RegExp(`var\\(${esc}[a-zA-Z0-9_-]+`, 'g'),
    // 按类别的 token 使用
    tokenColor: new RegExp(`var\\(${esc}(?:g-(?:fg|bg|border)-color|primary|blue|black|white|red|orange|green|grey|yellow)[a-zA-Z0-9_-]*`, 'g'),
    tokenSpacing: new RegExp(`var\\(${esc}(?:g-spacing|s-)[a-zA-Z0-9_.-]*`, 'g'),
    tokenRadius: new RegExp(`var\\(${esc}(?:g-radius|r-)[a-zA-Z0-9_.-]*`, 'g'),
    tokenShadow: new RegExp(`var\\(${esc}(?:g-shadow|shadow)[a-zA-Z0-9_-]*`, 'g'),
    tokenTypography: new RegExp(`var\\(${esc}(?:typography|g-text)[a-zA-Z0-9_-]*`, 'g'),
    tokenTransition: new RegExp(`var\\(${esc}(?:transition|g-transition)[a-zA-Z0-9_-]*`, 'g'),
...
let RE = buildRegexes(CONFIG.tokenPrefix);
...
function classifyColorContext(line, hex) {
  // 动态构建 token 定义检测正则（基于 CONFIG.tokenAliases）
  const aliasPattern = CONFIG.tokenAliases
    .map(a => a.slice(2).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))  // 去掉 -- 并转义
    .join('|');
  const tokenDefRe = new RegExp(`--(?:${aliasPattern})[a-zA-Z0-9_-]+\\s*:\\s*#`, 'i');
  if (tokenDefRe.test(line)) return 'token_def';

  // CSS fallback: var(--*, #495dff)
...
用法:
  ...
  node token-scan.js <目录> --config spec.json         从 spec 文件读取 Token 定义
  node token-scan.js <目录> --config walkthrough.json  从走查配置读取（自动定位 spec）
  node token-scan.js <目录> --token-prefix --my-       自定义 Token 变量前缀
  node token-scan.js <目录> --html                     同时生成 HTML 报告
  node token-scan.js <目录> --json out.json            输出 JSON 到文件
...
配置优先级:
  --token-prefix > --config 中的 tokenPrefix > 默认值 (--lg-)
  --config spec.json > 默认内置值（灵基 Token）
...
示例:
  ...
  # 使用走查配置文件（自动读取关联的 spec）
  node token-scan.js ./extracted --config walkthrough-config.json --html

  # 自定义 Token 前缀的项目
  node token-scan.js ./src --token-prefix --ds- --config my-spec.json --html
`);
    process.exit(0);
  }
  ...
  const prefixIdx = args.indexOf('--token-prefix');
  const customPrefix = prefixIdx >= 0 ? args[prefixIdx + 1] : null;

  // ── 加载配置 ──
  if (configPath) {
    const specOverrides = loadFromSpec(configPath);
    CONFIG = { ...DEFAULT_CONFIG, ...specOverrides };
    console.error(`▶ 已加载配置：${configPath}`);
  }
  if (customPrefix) {
    CONFIG.tokenPrefix = customPrefix;
    // 自动添加别名（将自定义前缀加入 aliases）
    if (!CONFIG.tokenAliases.includes(customPrefix)) {
      CONFIG.tokenAliases = [customPrefix, ...CONFIG.tokenAliases];
    }
    console.error(`▶ Token 前缀：${customPrefix}`);
  }
  // 根据最终配置重建正则
  RE = buildRegexes(CONFIG.tokenPrefix);
  ...
  console.error(`▶ Token 前缀：${CONFIG.tokenPrefix}，已知色值：${CONFIG.knownTokenColors.length} 个`);
